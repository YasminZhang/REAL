...
class COTRewardTrainer(RayPPOTrainer):
    ...
    This trainer extends RayPPOTrainer to compute CoT likelihood ratios during rollouts.
    It computes π_θ(a|c,x) / π_θ(a|x) where:
    - a: ground truth answer
    - c: chain of thought reasoning extracted from response
    - x: original question/prompt
    - π_θ: current model being trained
    
    The log probabilities are computed during rollouts and passed through DataProto
    ...
    def __init__(
        ...
        # CoT reward configuration
        self.cot_config = CoTRewardConfig(
            delimiter=getattr(self.config.algorithm, 'cot_delimiter', '\\boxed{'),
            min_cot_length=getattr(self.config.algorithm, 'cot_min_length', 10),
            max_ratio=getattr(self.config.algorithm, 'cot_max_ratio', 10.0),
            log_rewards=getattr(self.config.algorithm, 'cot_log_rewards', True)
        )
        
        # CoT log probability computation configuration
        self.cot_logprob_config = CoTLogProbConfig(
            delimiter=self.cot_config.delimiter,
            max_sequence_length=getattr(self.config.data, 'max_prompt_length', 1024) + 
                               getattr(self.config.data, 'max_response_length', 8192),
            truncate_tokens=getattr(self.config.algorithm, 'cot_truncate_tokens', 50)
        )
        
        print(f"CoT Reward Trainer initialized with delimiter: {self.cot_config.delimiter}")
        print(f"Max sequence length: {self.cot_logprob_config.max_sequence_length}")
    ...
    def _compute_and_store_cot_log_probs(self, batch: DataProto) -> DataProto:
        """
        Compute CoT log probabilities using efficient batch processing and store in DataProto.
        
        This method:
        1. Uses the existing actor worker infrastructure to compute log probabilities
        2. Handles response splitting, padding, and attention mask management
        3. Computes both π_θ(a|c,x) and π_θ(a|x) efficiently 
        4. Stores results in non_tensor_batch for reward function access
